...
1. **Requisitos:** las 6 ACs implementadas y verificadas end-to-end contra el stack Docker real (no solo tests unitarios): login correcto/incorrecto, cookie firmada+httpOnly+secure-en-prod, redirección de rutas protegidas, password vía env var, rate limiting.
2. **Estándares de código:** `lib/config.ts` es la única puerta a `process.env` (getters perezosos, ver nota de bug); Route Handler de login valida con `zod`; naming conventions seguidas; sin nuevos warnings de lint.
3. **Testing:** 3 tests de integración (`auth.test.ts`: clave correcta, clave incorrecta, rate limit) + 2 de middleware — los 5 exigidos por la historia (y uno extra de rate limit) pasando junto con el resto de la suite (8/8).
4. **Funcionalidad verificada manualmente:** sí — flujo completo probado con `curl` contra `https://localhost` vía Docker Compose (login, acceso con/sin cookie, logout), no solo por inspección de código.
5. **Administración:** las 6 tasks marcadas `[x]`, Dev Agent Record y Change Log actualizados.
6. **Dependencias/build:** `npm run build`, `npm run lint`, `npm run test` limpios; `zod` fijado en versión exacta; `docker compose build app` reconstruido y verificado tras ambos fixes (Web Crypto, getters perezosos).
7. **Documentación:** Completion Notes documentan ambos bugs reales encontrados y su causa raíz, no solo el resultado final.

**Gaps conocidos:** ninguno para esta historia — a diferencia de 1.1, todo lo que pide esta historia es verificable en este entorno de desarrollo (no depende de infraestructura externa real).
